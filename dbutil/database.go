...
func ParseDialect(engine string) (Dialect, error) {
	...
	if strings.HasPrefix(engine, "postgres") || engine == "pgx" {
		...
		return SQLite, nil
	} else if strings.HasPrefix(engine, "sqlserver") || strings.HasPrefix(engine, "mssql") {
		return MSSQL, nil
	} else {
...
func replacePositionalParams(query, character string) string {
...
func (db *Database) mutateQuery(query string) string {
	...
	case MSSQL:
		return replacePositionalParams(query, "@p")
	default:
		return query
	}
}
